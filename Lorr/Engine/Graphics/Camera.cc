...
namespace lr::Graphics
{
    void Camera3D::Init(Camera3DDesc *pDesc)
    {
        ...
        m_ViewDirection =
            XMVectorSet(pDesc->m_ViewDirection.x, pDesc->m_ViewDirection.y, pDesc->m_ViewDirection.z, 0.0);
        m_UpDirection =
            XMVectorSet(pDesc->m_UpDirection.x, pDesc->m_UpDirection.y, pDesc->m_UpDirection.z, 0.0);
    ...
    void Camera3D::CalculateView()
    {
        ...
        XMVECTOR posDir = XMVectorAdd(m_Position, m_ViewDirection);
        m_View = XMMatrixLookAtRH(m_Position, posDir, m_UpDirection);
    }
    ...
    void Camera3D::SetPosition(const XMFLOAT3 &position)
    {
        ...
        m_Position = XMVectorSet(position.x, position.y, position.z, 0.0);
    }
    ...
    void Camera3D::SetDirectionAngle(XMFLOAT2 angle)
    {
        ...
        m_ViewDirection =
            XMVector3Normalize(XMVector3TransformCoord(XMVectorSet(0.0, 0.0, 1.0, 0.0), rotationMatrix));
    ...
    void Camera3D::Update(f32 deltaTime, f32 movementSpeed)
    {
        ...
        m_ResultMatrix =
            XMMatrixMultiply(XMMatrixInverse(nullptr, m_Projection), XMMatrixInverse(nullptr, m_View));
    }
    ...
    void Camera2D::Update(f32 deltaTime, f32 movementSpeed)
    {
        ...
        m_ResultMatrix = XMMatrixMultiplyTranspose(m_View, m_Projection);
    }
