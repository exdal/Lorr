...
void CommandList::set_push_constants(
    PipelineLayout *layout, void *data, u32 data_size, u32 offset, ShaderStageFlag stage_flags)
{
    ZoneScoped;

    vkCmdPushConstants(
        m_handle, layout->m_handle, static_cast<VkShaderStageFlags>(stage_flags), offset, data_size, data);
}
...
void CommandList::set_index_buffer(Buffer *buffer, u64 offset, bool use_u16)
{
    ...
    vkCmdBindIndexBuffer(
        m_handle, buffer->m_handle, offset, use_u16 ? VK_INDEX_TYPE_UINT16 : VK_INDEX_TYPE_UINT32);
}
...
void CommandList::draw_indexed(
    u32 index_count, u32 first_index, i32 vertex_offset, u32 instance_count, u32 first_instance)
{
    ZoneScoped;
...
void CommandBatcher::insert_image_barrier(ImageBarrier &barrier)
{
    ...
    barrier.image = *m_device->get_image(image_id);
