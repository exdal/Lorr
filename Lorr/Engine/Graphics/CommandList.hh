...
struct CommandAllocatorInfo {
    ...
    vk::CommandAllocatorFlag flags = vk::CommandAllocatorFlag::None;
    std::string_view debug_name = {};
};

struct CommandAllocator {
    vk::CommandType type = vk::CommandType::Count;
    ls::option<vk::CommandAllocator> handle = ls::nullopt;

    operator auto &() { return handle; }
    operator const auto &() const { return handle; }
    explicit operator bool() { return handle.has_value(); }
};
...
struct CommandList {
    ...
    void set_descriptor_sets(
        this CommandList &, PipelineLayoutID layout_id, PipelineBindPoint bind_point, u32 first_set, ls::span<DescriptorSet> sets);
    ...
    void draw_indexed(
        this CommandList &, u32 index_count, u32 first_index = 0, i32 vertex_offset = 0, u32 instance_count = 1, u32 first_instance = 0);
...
struct CommandQueue {
    ...
    [[nodiscard]] CommandList &begin_command_list(
        this CommandQueue &, usize frame_index, std::source_location LOC = std::source_location::current());
