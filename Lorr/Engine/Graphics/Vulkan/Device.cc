...
auto Device::destroy(this Device &) -> void {
}
...
auto Device::new_frame(this Device &self, SwapChain &swap_chain) -> vuk::Value<vuk::ImageAttachment> {
    ZoneScoped;

    self.runtime->next_frame();
    auto acquired_swap_chain = vuk::acquire_swapchain(swap_chain.handle_.value());
    auto acquired_image = vuk::acquire_next_image("present_image", std::move(acquired_swap_chain));
...
auto Device::end_frame(this Device &self, vuk::Value<vuk::ImageAttachment> &&target_attachment) -> void {
    ...
    result.submit(*self.transfer_manager.frame_allocator, self.compiler, {});
