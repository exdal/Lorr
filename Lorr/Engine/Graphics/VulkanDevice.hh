...
public:
    ...
    [[nodiscard]] auto alloc_transient_buffer_raw(
        ...
        usize size,
        LR_THISCALL) -> vuk::Buffer;
    [[nodiscard]] auto alloc_transient_buffer(
        this TransferManager &,
        vuk::MemoryUsage usage,
        usize size,
        LR_THISCALL) -> vuk::Value<vuk::Buffer>;
    ...
    [[nodiscard]] auto upload_staging(
        ...
        void *data,
        u64 data_size,
        LR_THISCALL) -> vuk::Value<vuk::ImageAttachment>;
    template<typename T>
    [[nodiscard]] auto upload_staging(
    ...
    template<typename T>
    [[nodiscard]] auto scratch_buffer(const T &val = {}, LR_THISCALL)
    ...
    auto wait_on(this TransferManager &, vuk::UntypedValue &&fut)
        -> void;
...
protected:
    [[nodiscard]] auto scratch_buffer(
        ...
        u64 size,
        LR_THISCALL) -> vuk::Value<vuk::Buffer>;
    auto wait_for_ops(this TransferManager &, vuk::Compiler &compiler)
        -> void;

    auto acquire(
        this TransferManager &, vuk::DeviceFrameResource &allocator)
        -> void;
...
struct DeviceResources {
    ...
    SlotMap<vuk::Unique<vuk::ImageView>, ImageViewID>
        image_views = {};
...
private:
    ...
    ankerl::unordered_dense::
        map<vuk::Name, ls::pair<vuk::Query, vuk::Query>>
            pass_queries = {};
...
public:
    auto init(this Device &, usize frame_count)
        -> std::expected<void, vuk::VkException>;
    auto destroy(this Device &) -> void;

    auto new_slang_session(
        this Device &, const SlangSessionInfo &info)
        -> ls::option<SlangSession>;

    auto transfer_man(this Device &) -> TransferManager &;
    auto new_frame(this Device &, vuk::Swapchain &)
        -> vuk::Value<vuk::ImageAttachment>;
    auto end_frame(
        this Device &,
        vuk::Value<vuk::ImageAttachment> &&target_attachment) -> void;
    auto wait(
        this Device &,
    ...
    auto create_persistent_descriptor_set(
        this Device &,
        ls::span<BindlessDescriptorInfo> bindings,
        u32 index) -> vuk::Unique<vuk::PersistentDescriptorSet>;
    auto commit_descriptor_set(
        this Device &, vuk::PersistentDescriptorSet &set) -> void;
    auto create_swap_chain(
        this Device &,
        VkSurfaceKHR surface,
        ls::option<vuk::Swapchain> old_swap_chain = ls::nullopt)
        -> std::expected<vuk::Swapchain, vuk::VkException>;

    auto set_name(
        this Device &, Buffer &buffer, std::string_view name) -> void;
    auto set_name(this Device &, Image &image, std::string_view name)
        -> void;
    auto set_name(
        this Device &, ImageView &image_view, std::string_view name)
        -> void;
    ...
    auto pipeline(this Device &, PipelineID)
        -> vuk::PipelineBaseInfo **;
