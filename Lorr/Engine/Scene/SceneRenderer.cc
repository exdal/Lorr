...
auto SceneRenderer::setup_persistent_resources(this SceneRenderer &self)
    -> void {
    ZoneScoped;
    ...
    BindlessDescriptorInfo descriptor_infos[] = {
        {.binding = Descriptor_Images,
         .type = vuk::DescriptorType::eSampledImage,
         .descriptor_count = 1024},
        {.binding = Descriptor_Sampler,
         .type = vuk::DescriptorType::eSampler,
         .descriptor_count = 1024},
    };
    self.bindless_set =
        self.device->create_persistent_descriptor_set(descriptor_infos, 1)
            .release();
    ...
    auto transmittance_lut_pass = vuk::make_pass(
        "transmittance lut",
        [&pipeline =
             *self.device->pipeline(self.sky_transmittance_pipeline.id())](
    ...
    auto multiscatter_lut_pass = vuk::make_pass(
        "multiscatter lut",
        [&pipeline =
             *self.device->pipeline(self.sky_multiscatter_pipeline.id())](
            vuk::CommandBuffer &cmd_list,
    ...
    auto transmittance_lut_attachment = self.sky_transmittance_lut_view.discard(
        *self.device,
        "sky transmittance lut",
        vuk::ImageUsageFlagBits::eStorage);
    auto multiscatter_lut_attachment = self.sky_multiscatter_lut_view.discard(
        *self.device,
        "sky multiscatter lut",
        vuk::ImageUsageFlagBits::eStorage);

    auto temp_scene_info = GPU::Scene{};
    temp_scene_info.atmosphere.transmittance_lut_size =
        self.sky_transmittance_lut_view.extent();
    temp_scene_info.atmosphere.multiscattering_lut_size =
    ...
    std::tie(transmittance_lut_attachment, temp_scene) = transmittance_lut_pass(
        std::move(transmittance_lut_attachment), std::move(temp_scene));
    std::tie(
        transmittance_lut_attachment, multiscatter_lut_attachment, temp_scene) =
    ...
    auto cloud_noise_lut_pass = vuk::make_pass(
        ...
        [&pipeline = *self.device->pipeline(self.cloud_noise_pipeline.id())](
            ...
            VUK_BA(vuk::Access::eComputeRead) scene) {
            auto image_size = glm::vec2(
                shape_noise_lut->extent.width, shape_noise_lut->extent.height);
            auto groups = glm::uvec2(image_size) / 16_u32;
            cmd_list  //
                .bind_compute_pipeline(pipeline)
    ...
    auto cloud_shape_noise_lut_attachment =
        self.cloud_shape_noise_lut_view.discard(
            *self.device, "cloud shape lut", vuk::ImageUsageFlagBits::eStorage);
    auto cloud_detail_noise_lut_attachment =
        self.cloud_detail_noise_lut_view.discard(
            *self.device,
            "cloud detail lut",
            vuk::ImageUsageFlagBits::eStorage);

    std::tie(
        cloud_shape_noise_lut_attachment,
        cloud_detail_noise_lut_attachment,
        temp_scene) =
        cloud_noise_lut_pass(
    ...
    self.device->set_name(
        self.sky_transmittance_lut_view, "Sky Transmittance LUT View");
    self.device->set_name(self.sky_multiscatter_lut, "Sky Multiscattering LUT");
    self.device->set_name(
        self.sky_multiscatter_lut_view, "Sky Multiscattering LUT View");
    self.device->set_name(self.cloud_shape_noise_lut, "Cloud Shape Noise");
    self.device->set_name(
        self.cloud_shape_noise_lut_view, "Cloud Shape Noise View");
    self.device->set_name(self.cloud_detail_noise_lut, "Cloud Detail Noise");
    self.device->set_name(
        self.cloud_detail_noise_lut_view, "Cloud Detail Noise View");
}

auto SceneRenderer::compose(
    this SceneRenderer &self, SceneComposeInfo &compose_info) -> ComposedScene {
    ZoneScoped;
    ...
    for (u32 i = 0; i < compose_info.image_view_ids.size(); i++) {
        auto *image_view =
            self.device->image_view(compose_info.image_view_ids[i]);
        auto *sampler = self.device->sampler(compose_info.samplers[i]);
        self.bindless_set.update_sampled_image(
            Descriptor_Images,
            i,
            *image_view,
    ...
    if (ls::size_bytes(compose_info.gpu_materials) >
        self.materials_buffer.data_size()) {
        ...
        self.materials_buffer =
            Buffer::create(
                *self.device, ls::size_bytes(compose_info.gpu_materials))
                .value();
    }

    if (ls::size_bytes(compose_info.gpu_models) >
        self.models_buffer.data_size()) {
        ...
        self.models_buffer =
            Buffer::create(
                *self.device, ls::size_bytes(compose_info.gpu_models))
                .value();
    }

    if (ls::size_bytes(compose_info.gpu_meshlet_instances) >
        self.meshlet_instances_buffer.data_size()) {
        ...
        self.meshlet_instances_buffer =
            Buffer::create(
                *self.device,
                ls::size_bytes(compose_info.gpu_meshlet_instances))
                .value();
    }
    ...
    auto materials_buffer =  //
        transfer_man.upload_staging(
            ls::span(compose_info.gpu_materials), self.materials_buffer);
    auto models_buffer =  //
        transfer_man.upload_staging(
            ls::span(compose_info.gpu_models), self.models_buffer);
    auto meshlet_instances_buffer =  //
        transfer_man.upload_staging(
            ls::span(compose_info.gpu_meshlet_instances),
            self.meshlet_instances_buffer);
...
auto SceneRenderer::render(
    ...
    if (info.transforms.size_bytes() > self.transforms_buffer.data_size()) {
        ...
        self.transforms_buffer = Buffer::create(
                                     *self.device,
                                     info.transforms.size_bytes(),
    ...
    auto transforms_buffer = self.transforms_buffer.acquire(
        *self.device, "Transforms Buffer", vuk::Access::eMemoryRead);
    if (!info.dirty_transform_ids.empty()) {
        auto transform_count = rebuild_transforms
                                   ? info.transforms.size()
                                   : info.dirty_transform_ids.size();
        auto new_transforms_size_bytes =
            transform_count * sizeof(GPU::Transforms);

        auto upload_buffer = transfer_man.alloc_transient_buffer(
            vuk::MemoryUsage::eCPUonly, new_transforms_size_bytes);
        auto *dst_transform_ptr =
            reinterpret_cast<GPU::Transforms *>(upload_buffer->mapped_ptr);
        auto upload_offsets = std::vector<u64>(transform_count);

        if (!rebuild_transforms) {
            for (const auto &[dirty_transform_id, offset] :
                 std::views::zip(info.dirty_transform_ids, upload_offsets)) {
                auto index = SlotMap_decode_id(dirty_transform_id).index;
                const auto &transform = info.transforms[index];
                std::memcpy(
                    dst_transform_ptr, &transform, sizeof(GPU::Transforms));
            ...
            std::memcpy(
                dst_transform_ptr,
                info.transforms.data(),
                ls::size_bytes(info.transforms));
        }
        ...
        auto update_transforms_pass = vuk::make_pass(
            ...
            [upload_offsets = std::move(upload_offsets)](
                ...
                for (usize i = 0; i < upload_offsets.size(); i++) {
                    ...
                    auto src_subrange = src_buffer->subrange(
                        i * sizeof(GPU::Transforms), sizeof(GPU::Transforms));
                    auto dst_subrange =
                        dst_buffer->subrange(offset, sizeof(GPU::Transforms));
        ...
        transforms_buffer = update_transforms_pass(
            std::move(upload_buffer), std::move(transforms_buffer));
    }

    info.scene_info.atmosphere.sky_view_lut_size = self.sky_view_lut_extent;
    info.scene_info.atmosphere.aerial_perspective_lut_size =
        self.sky_aerial_perspective_lut_extent;
    info.scene_info.atmosphere.transmittance_lut_size =
        self.sky_transmittance_lut_view.extent();
    info.scene_info.atmosphere.multiscattering_lut_size =
        self.sky_multiscatter_lut_view.extent();
    info.scene_info.clouds.noise_lut_size =
        self.cloud_shape_noise_lut_view.extent();
    ...
    auto final_attachment = vuk::declare_ia(
        ...
        {.extent = info.extent,
         ...
         .layer_count = 1});
    final_attachment =
        vuk::clear_image(std::move(final_attachment), vuk::Black<f32>);
    ...
    auto depth_attachment = vuk::declare_ia(
        ...
        {.usage = vuk::ImageUsageFlagBits::eSampled |
         ...
         .layer_count = 1});
    depth_attachment =
        vuk::clear_image(std::move(depth_attachment), vuk::ClearDepth(1.0));
    ...
    auto sky_view_pass = vuk::make_pass(
        ...
        [&pipeline = *self.device->pipeline(self.sky_view_pipeline.id())](
            ...
            return std::make_tuple(
                dst, transmittance_lut, multiscatter_lut, scene);
        });
    ...
    auto sky_aerial_perspective_pass = vuk::make_pass(
        "sky aerial perspective",
        [&pipeline =
             *self.device->pipeline(self.sky_aerial_perspective_pipeline.id())](
            vuk::CommandBuffer &cmd_list,
            ...
            auto image_size = glm::vec3(
                dst->extent.width, dst->extent.height, dst->extent.depth);
            auto groups = glm::uvec2(image_size) / 16_u32;
            cmd_list  //
                .bind_compute_pipeline(pipeline)
                .bind_sampler(0, 0, sampler_info)
    ...
    visbuffer_attachment = vuk::clear_image(
        std::move(visbuffer_attachment),
        vuk::Clear(vuk::ClearColor(~0_u32, 0_u32, 0_u32, 0_u32)));
    ...
    albedo_attachment =
        vuk::clear_image(std::move(albedo_attachment), vuk::Black<f32>);
    ...
    normal_attachment =
        vuk::clear_image(std::move(normal_attachment), vuk::Black<f32>);
    ...
    emissive_attachment =
        vuk::clear_image(std::move(emissive_attachment), vuk::Black<f32>);
    ...
    if (self.meshlet_instance_count) {
        ...
        auto vis_cull_meshlets_pass = vuk::make_pass(
            "vis cull meshlets",
            [&pipeline =
                 *self.device->pipeline(self.vis_cull_meshlets_pipeline.id()),
             meshlet_instance_count = self.meshlet_instance_count](
                ...
                cmd_list  //
                    ...
                    .dispatch(
                        (meshlet_instance_count + Model::MAX_MESHLET_INDICES -
                         1) /
                        Model::MAX_MESHLET_INDICES);
                return std::make_tuple(
                    scene, meshlet_indirect, visible_meshlet_instances_indices);
            });

        auto meshlets_indirect_dispatch =
            transfer_man.scratch_buffer<vuk::DispatchIndirectCommand>(
                {.x = 0, .y = 1, .z = 1});
        auto visible_meshlet_instances_buffer =
            transfer_man.alloc_transient_buffer(
                vuk::MemoryUsage::eGPUonly,
                self.meshlet_instance_count * sizeof(u32));
        ...
        auto vis_cull_triangles_pass = vuk::make_pass(
            "vis cull triangles",
            [&pipeline =
                 *self.device->pipeline(self.vis_cull_triangles_pipeline.id())](
                vuk::CommandBuffer &cmd_list,
        ...
        auto triangles_indexed_indirect_dispatch =
            transfer_man.scratch_buffer<vuk::DrawIndexedIndirectCommand>(
                {.instanceCount = 1});
        auto reordered_indices_buffer = transfer_man.alloc_transient_buffer(
            vuk::MemoryUsage::eGPUonly,
            self.meshlet_instance_count * Model::MAX_MESHLET_PRIMITIVES * 3 *
                sizeof(u32));
        ...
        if (composed_scene.has_value()) {
            ...
            meshlet_instances_buffer =
                std::move(composed_scene->meshlet_instances_buffer);
        } else {
            materials_buffer = self.materials_buffer.acquire(
                *self.device, "Materials", vuk::Access::eNone);
            models_buffer = self.models_buffer.acquire(
                *self.device, "Models", vuk::Access::eNone);
            meshlet_instances_buffer = self.meshlet_instances_buffer.acquire(
                *self.device, "Meshlet Instances", vuk::Access::eNone);
        }
             ...
             &descriptor_set = self.bindless_set](
                ...
                cmd_list  //
                    ...
                    .push_constants(
                        ...
                        0,
                        glm::vec2(
                            visbuffer->extent.width, visbuffer->extent.height))
                    .draw(3, 1, 0, 1);
                return std::make_tuple(
                    albedo, normal, emissive, scene, transforms);
            });
    ...
    if (info.render_clouds) {
        //  ── CLOUDS ──────────────────────────────────────────────────────────
        auto cloud_shape_noise_lut_attachment =
            self.cloud_shape_noise_lut_view.acquire(
                *self.device,
                "cloud shape lut",
                vuk::ImageUsageFlagBits::eSampled,
                vuk::Access::eFragmentSampled);
        auto cloud_detail_noise_lut_attachment =
            self.cloud_detail_noise_lut_view.acquire(
                *self.device,
                "cloud detail lut",
                vuk::ImageUsageFlagBits::eSampled,
                vuk::Access::eFragmentSampled);
        ...
        auto cloud_apply_pass = vuk::make_pass(
            "cloud apply",
            [&pipeline =
                 *self.device->pipeline(self.cloud_apply_pipeline.id())](
                vuk::CommandBuffer &cmd_list,
    ...
    auto editor_grid_pass = vuk::make_pass(
        ...
        [&pipeline = *self.device->pipeline(self.grid_pipeline.id())](
            ...
            VUK_BA(vuk::Access::eVertexRead | vuk::Access::eFragmentRead)
                scene) {
            cmd_list  //
                .bind_graphics_pipeline(pipeline)
